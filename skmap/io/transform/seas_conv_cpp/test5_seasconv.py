...
if new_flag:
    ...
    ts_p = np.zeros((N_pix,N_ext), dtype=np.float32)
    ts_p[:,0:N_img] = ts.astype(np.float32).copy()
    qa_p = np.zeros((N_pix,N_ext), dtype=np.float32)
...
if cmp_flag:
    ...
    print(f"Relative error float vs. double {rel_err:.8f}")